...
void initVulkan()
{
    ...
    auto triangleVS = loadShader(device, "shaders/blend.vert.spv");
    assert(triangleVS);
    auto triangleFS = loadShader(device, "shaders/blend.frag.spv");
    assert(triangleFS);
...
void destroyVulkan()
{
    ...
    vkDestroySampler(device, texture0->textureSampler, nullptr);
    vkDestroyImageView(device, texture0->textureImageView, nullptr);
    vkDestroyImage(device, texture0->textureImage, nullptr);
    vkFreeMemory(device, texture0->textureImageMemory, nullptr);
...
void createDescriptorPool(VkDevice device)
{
    ...
    poolSizeSampler2.descriptorCount = static_cast<uint32_t>(swapchainMinImageCount);
...
void createTextureSampler(VkSampler &textureSampler)
{
    ...
    samplerInfo.maxAnisotropy = physicalDevProps.limits.maxSamplerAnisotropy;
